...
import (
	...
	tree_sitter_sql "github.com/ngavinsir/treesittergo/tree-sitter-sql/bindings/go"
...
func BenchmarkTreesittergo(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		tree, err := p.ParseStringCtx(ctx, q)
		if err != nil {
			panic(err)
		}
		_ = tree.RootNodeCtx(ctx)
		// fmt.Println(root.StringCtx(ctx))
	}
}
...
func BenchmarkGoTreesitter(b *testing.B) {
	...
	defer parser.Close()
	parser.SetLanguage(tree_sitter.NewLanguage(tree_sitter_sql.Language()))

	for i := 0; i < b.N; i++ {
		tree := parser.Parse([]byte(q), nil)
		defer tree.Close()

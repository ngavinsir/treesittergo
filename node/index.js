...
const importObject = { wasi_snapshot_preview1: wasi.wasiImport, env: { emscripten_notify_memory_growth: () => { } } };

const q = `select time_id, product
...
async function getRootStr() {
  const wasm = await WebAssembly.compile(
    fs.readFileSync("./ts-combined-sql.wasm")
  );
  const instance = await WebAssembly.instantiate(wasm, importObject);

  const p = instance.exports.ts_parser_new();
  const sqlLang = instance.exports.tree_sitter_sql();
  // const langVersion = instance.exports.ts_language_version(sqlLang);
  instance.exports.ts_parser_set_language(p, sqlLang);
  // const langNamePtr = instance.exports.ts_language_name(sqlLang);
  // const langNameSize = instance.exports.strlen(langNamePtr);
  // const langName = new TextDecoder().decode(new Uint8Array(instance.exports.memory.buffer, langNamePtr, langNameSize))
  // console.log({ langVersion, langName });

  const strByte = new TextEncoder().encode(q);
  const strPtr = instance.exports.malloc(strByte.length);
  new Uint8Array(instance.exports.memory.buffer, strPtr, strByte.length).set(strByte)
  const tree = instance.exports.ts_parser_parse_string(p, 0, strPtr, strByte.length)
  // const str = new TextDecoder().decode(new Uint8Array(instance.exports.memory.buffer, strPtr, strByte.length));
  // console.log({ str });

  const rootPtr = instance.exports.malloc(24);
  instance.exports.ts_tree_root_node(rootPtr, tree);
  const rootStrPtr = instance.exports.ts_node_string(rootPtr);
  const strSize = instance.exports.strlen(rootStrPtr);
  const rootStr = new TextDecoder().decode(new Uint8Array(instance.exports.memory.buffer, rootStrPtr, strSize))

  // console.log({ e: instance.exports })
  // console.log({ rootPtr, strSize, rootStr: rootStr.length })
};
...
b.suite(
  ...
  b.cycle(),
  b.complete(),
  b.save({ file: 'js-node-wasm', version: '1.0.0', details: true }),
)
